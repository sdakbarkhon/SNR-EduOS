...
export async function createLibraryMaterial(
   ...
   *
   * Теперь вызывающий передаёт выбранный предмет явно — из того же списка
   * fn_my_subject_slugs(), по которому судит база. Карточка осталась запасным
   * вариантом для старых вызовов.
   *
   * ═══ ЗДЕСЬ БОЛЬШЕ НЕТ ПРОВЕРКИ ПРАВА (04.09.2026) ═══════════════════════
   *
   * Стоял отказ «Куратор (без привязки к предмету) не может загружать» — по
   * колонке карточки. Он был ВТОРОЙ КОПИЕЙ правила и притом строже базы:
   * fn_my_subject_slugs умеет взять слаг у однопредметного коллеги, а карточка
   * про коллегу не знает. Роли куратора в продукте тоже нет с 30.08.2026.
   *
   * Право живёт в политике вставки (миграции 190/191) и только там. Ниже
