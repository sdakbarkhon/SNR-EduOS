 ...
 *
 * СМЕЩЕНИЕ (`offset`) добавлено 30.08.2026. До него лента бралась ОДНИМ
 * куском с пределом, и всё, что за пределом, было недостижимо: человек
 * видел на колокольчике число, за которым стояли записи, до которых нельзя
 * дойти. Теперь страница берётся окном `range(offset, offset + limit - 1)`.
 *
 * Почему окно, а не растущий предел. Веб у ученика и учителя подгружает
 * иначе: перезапрашивает ВЕСЬ список с бо́льшим пределом (20, 40, 60…).
 * Это работает, но на тысяче строк пятидесятое нажатие тянет тысячу строк
 * ради двадцати новых. Окно тянет ровно страницу, сколько бы их ни было.
 *
 * При `offset = 0` запрос СОБИРАЕТСЯ КАК ПРЕЖДЕ — `.limit()`, не `.range()`.
 * Так все, кто зовёт функцию двумя доводами (веб ученика, учителя,
 * родителя, колокольчик), получают ровно тот же запрос, что и до правки.
 *
 * Порядок устойчив: `created_at DESC` по индексу. Новая запись во время
 * листания сдвигает окно на строку вниз, то есть даёт ПОВТОР уже виденной
 * записи, а не пропуск — экран склеивает страницы с проверкой по id.
 */
