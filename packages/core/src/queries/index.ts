...
export const getStudentLessonView = async (
  ...
  const schoolQuery = db3
    .from("schools")
    .select("autostart_enabled, is_demo")
    .eq("id", lesson.school_id)
    .maybeSingle();
  // Большой фикс, Блок 3 — правило 3-го урока действует ТОЛЬКО в демо-школе
  // (см. StudentLessonView.isThirdLessonViewer). Раньше «демо ли школа»
  // решалось вписанным идентификатором прямо здесь; теперь признак один на
  // проект — schools.is_demo, он приходит соседним запросом выше.
  //
  // Порядковый номер дня спрашиваем ВСЕГДА, а не только у демо-школы: узнать
  // признак до запуска Promise.all нельзя, не разбив параллельность, а
  // fn_lesson_day_index — один дешёвый вызов. Результат применяется ниже
  // только если школа демо, поэтому поведение реальных школ не меняется.
  const dayIndexQuery = db3.rpc("fn_lesson_day_index", { p_lesson_id: lessonId });
  ...
  const schoolTyped = schoolRes as {
    data: { autostart_enabled: boolean; is_demo: boolean } | null;
    error: { message: string } | null;
  };
  ...
  return {
    ...
    // Демо ли школа — из признака schools.is_demo, а не из вписанного номера.
    isThirdLessonViewer: Boolean(schoolTyped.data?.is_demo) && (dayIndexTyped.data ?? 1) >= 3,
  };
};
 ...
 * 11.09.2026 — им же экран урока ученика разрешает в демо листать презентацию
 * у себя и выходить из неё (SlideViewer.tsx canNavigate, LessonWorkspaceView.tsx
 * presentationLocked).
 *
 ...
 * Правка урока.
 *
 * 30.08.2026, пункт 78 — ПРЕДМЕТ ТЕПЕРЬ ПРИНИМАЕТСЯ. Его тут не было, и
 * запрета на это в решениях не нашлось: просто не сделали. Последствий было
 * два, и второе хуже первого.
 *
 * ПЕРВОЕ. Ошибся предметом при создании — исправить нельзя ниоткуда,
 * остаётся удалить урок и завести заново, потеряв этапы, материалы,
 * посещаемость и оценки (всё это висит на lesson_id с ON DELETE CASCADE).
 *
 * ВТОРОЕ, И ОНО НАСТОЯЩЕЕ. Форма правки УЖЕ показывала селектор предмета —
 * он рисуется по наличию группы и в правке тоже. Учитель его видел, менял,
 * а выбор уходил в никуда: сюда он не доезжал. Поле, которое выглядит
 * рабочим и молча ничего не делает, хуже отсутствующего.
 *
 * ТРЕТЬЕ, ПОБОЧНОЕ И САМОЕ ТИХОЕ. group_id принимался, а subject_id — нет.
 * Перенос урока в другую группу оставлял предмет СТАРОЙ группы, и урок
 * становился рассогласованным: группа одна, предмет из другой. Проверено
 * прогоном — база это пропускает, ограничения «предмет принадлежит группе
 * урока» в схеме нет вовсе (только FK на subjects.id).
 *
 * ЧУЖОЙ ПРЕДМЕТ НЕ ПРОЙДЁТ, и это не наша заслуга: правило
 * «teacher updates own group lessons» держит is_subject_owner(subject_id) и
 * в USING (старый предмет), и в WITH CHECK (новый). Проверено прогоном:
 * попытка поставить чужой предмет отбивается кодом 42501.
 */
export const updateLesson = async (
  ...
  patch: {
    ...
    room?: string | null;
    group_id?: string;
    /** Предмет урока. Пустую строку не принимаем: у урока предмет
     *  обязателен с миграции 226, и «сбросить» его нельзя. */
    subject_id?: string;
  },
  ...
  // понятнее. Не передан вовсе — законно: значит правка предмета не про это.
  if (patch.subject_id !== undefined && !patch.subject_id) {
    throw new Error("У урока должен быть предмет");
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const createLesson = async (
  ...
  // УЧИТЕЛЬ НЕ МОЖЕТ БЫТЬ НА ДВУХ УРОКАХ РАЗОМ. 06.09.2026.
  //
  // Проверка стоит ЗДЕСЬ, а не в трёх вызывающих местах, именно потому, что
  // мест три: одиночная кнопка «Создать урок», массовое создание и создание
  // уроков по учебному плану — все проходят через createLesson. Разложи её по
  // ним, и через месяц одно место проверяло бы, а другое нет.
  //
  // Смотрит по ВСЕМ школам человека: функция базы отвечает поверх правила
  // доступа, потому что свои уроки в невыбранной школе учитель не видит и сам
  // (миграция 266). Служебному ключу функция отвечает пусто — фоновые заливки
  // в эту проверку не упрутся.
  //
  // Отказ по-русски и без названия школы: сказать «у вас урок в школе такой-то»
  // значило бы показать то, чего человек в этот момент не видит. Ему довольно
  // знать, что время занято им самим.
  const занятость = await getMyTeacherBusyBetween(
    db,
    input.startsAt,
    new Date(new Date(input.startsAt).getTime() + dur * 60 * 1000).toISOString(),
  );
  if (занятость.length > 0) {
    throw new Error("В это время у вас уже есть другой урок");
  }
...
/**
 * ЗАНЯТЫЕ ПРОМЕЖУТКИ САМОГО УЧИТЕЛЯ — ПО ВСЕМ ЕГО ШКОЛАМ. 06.09.2026.
 *
 * ЗАЧЕМ ОТДЕЛЬНО ОТ getGroupLessonsInDateRange. Тот отвечает «что стоит у этой
 * группы», и этого хватало, пока учитель работал в одной школе. С двумя
 * школами вопрос другой: «занят ли ЧЕЛОВЕК» — а его уроки в другой школе
 * обычным запросом не достать вовсе, правило доступа сузило `lessons` до
 * выбранной школы.
 *
 * ПОЭТОМУ ЧЕРЕЗ ФУНКЦИЮ БАЗЫ (миграция 266). Она отвечает только про
 * спрашивающего и смотрит поверх правила доступа. Своего запроса к `lessons`
 * здесь нет и быть не может: он вернул бы половину правды.
 *
 * Пустой ответ означает «не занят»: так отвечает функция и служебному ключу,
 * под которым учителя нет. Это осознанный выбор — молчание не должно
 * превращаться в запрет.
 *
 * И ПО ТОЙ ЖЕ ПРИЧИНЕ ОШИБКА НЕ БРОСАЕТСЯ. Отказ функции — недоступная база,
 * непринятая ещё миграция 266 — не должен запрещать создание урока: проверка
 * добавлена, чтобы ловить наложения, а не чтобы становиться новым способом
 * уронить создание. Не смогли спросить — пишем в лог и считаем свободным.
 */
export const getMyTeacherBusyBetween = async (
  db: Db,
  fromIso: string,
  toIso: string,
): Promise<Array<{ starts_at: string; ends_at: string | null }>> => {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data, error } = await (db as any)
      .rpc("fn_my_teacher_busy_between", { p_from: fromIso, p_to: toIso });
    if (error) throw new Error(error.message);
    return (data ?? []) as Array<{ starts_at: string; ends_at: string | null }>;
  } catch (e) {
    console.error("[занятость] спросить не вышло:", (e as Error)?.message);
    return [];
  }
};
