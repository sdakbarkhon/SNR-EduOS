...
/** Средний балл: сумма оценок, приведённых к пятибалльной, делённая на их
 *  число. Неоценённые работы (grade5 = null) в расчёт не входят вовсе — они
 *  не «ноль», их просто ещё не проверили. */
export function averageGrade(grades: AnalyticsGrade[]): number | null {
  const vals = grades.map((g) => g.grade5).filter((v): v is number => v != null);
