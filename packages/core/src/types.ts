...
export type Group = {
  id: string;
  name: string;
  /** ЗАГЛУШКА, а не предмет группы. С миграции 97 здесь у всех групп стоит
   *  programming (у двух — robotics и Схемотехника). Настоящий предмет
   *  живёт в таблице subjects строкой на пару (группа, учитель). Колонку
   *  читает форма группы в админке, и она с ней согласована; для подписи
   *  предмета на экране брать её НЕЛЬЗЯ. */
  subject: string;
  teacher_id: string | null;
  /** Дни недели строкой. Пусто у всех семи групп обеих школ — расписание
   *  ведётся уроками, а не этим полем. Optional: часть выборок её не берёт. */
...
export type Subject = {
  ...
  /** Миграция 108: заглушка каталога, а не настоящее назначение учителя.
   *  Такими помечены 13 строк куратора — «Биология», «ИЗО», «Музыка» и
   *  прочие предметы, которые в расписании не ведутся. Optional по той же
   *  причине, что и catalog_id: не все выборки её берут. */
  is_stub?: boolean;
  /** Миграция 98. Выключенный предмет каталога. */
  is_active?: boolean;
  created_at: string;
  updated_at: string;
};
...
export type StudentLessonView = {
   ...
   *  157): ученик не активирует этапы и не стартует/не завершает урок.
   *  Листать презентацию у себя и выходить из неё может (11.09.2026, см.
   *  SlideViewer.tsx canNavigate). Реальная школа и 1-2-й урок дня —
...
export type AttendanceRollCallRow = {
  ...
  is_finalized: boolean;
  // 23.08.2026 (миграция 225). Кто поставил отметку. null при заполненном
  // status = поставила машина: автозавершение урока раздало прогулы
  // неотмеченным. Такую отметку учитель вправе исправить и после запирания —
  // это его ПЕРВОЕ выставление, а не правка чужой проверки.
