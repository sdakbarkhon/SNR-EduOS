...
export default function ChatScreen() {
  ...
  // имя, и демо показывало разговор с человеком, которого в школе нет.
  // Берём первого предметника группы — переписка в демо одна.
  ...
  /**
   * 23.08.2026. Было: нажатие молча очищало поле — снаружи это выглядит
   * как «сообщение ушло». Чата родителя с учителем в школе не заведено,
   * поэтому кнопка объясняет это прямо и НЕ трогает набранный текст.
   */
  const onSend = () => {
    ...
    <AppBackground>
      ...
      >
        ...
        >
          ...
          {/* Пояснение вместо изображённой отправки. Текст — тот же, что
              видит настоящий родитель на «Скоро» этого раздела. */}
          {sendSoon ? (
            <View style={{ paddingBottom: 8 }}>
              <SoonNote text={d.parentApp.soon2.items.chat.text} />
            </View>
          ) : null}
