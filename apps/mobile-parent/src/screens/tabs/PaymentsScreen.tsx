...
import { BILL_META } from "../../data/demoPayments";
...
export default function PaymentsScreen() {
  ...
  // Ребёнок — ТОТ ЖЕ, что на остальных экранах раздела: выбранный
  // родителем настоящий ребёнок из useChildScope. Раньше здесь стоял
  // getSelectedChildContext() без аргумента, а он без явного id отдаёт
        ...
        <GlassCard contentStyle={{ paddingVertical: 4, paddingHorizontal: 14 }}>
          {dueBills.map((bill, i) => (
            <ListRow
              ...
              title={bill.title}
              subtitle={[who, BILL_META[bill.id]?.noteTail].filter(Boolean).join(" · ")}
              right={
                <View style={{ alignItems: "flex-end", gap: 2 }}>
                  ...
                  <Text style={{ fontFamily: fonts.manrope800, fontSize: 9.5, color: tokens.status.orange.text }}>
                    {fillTemplate(d.parentApp.pay.billDueBy, { date: fullDate(BILL_META[bill.id].dueDate, localeTag) })}
                  </Text>
                </View>
              }
              chevron
