 ...
 *   (15.08.2026: раньше был no-op и молчал; теперь ведёт на экран, который
 *   объясняет, что фильтр появится, когда предметов станет больше).
...
export default function AllSubjectsScreen() {
  ...
  // 15.08.2026 (заглушки). Кнопка была задокументированным no-op: нажималась
  // и молчала. Фильтровать пока нечего (предметов меньше десятка), но кнопка
  // из макета осталась — пусть она это и объясняет.
  const onFilter = () => {
    navigation.navigate("stub", { stubKey: "subjfilter" });
  };
