 ...
 *
 * ЗАХОД 5x · verify (заказчик, правка 2): все значения ниже — из §s10
 * «Токены · светлая / тёмная» макета, строки 2803–2818 (правая колонка «ТЁМНАЯ»)
 * + CSS-оверрайдов [data-snr-dark="1"] строк 23–190. Верифицировано «глаз-в-глаз»
 * с CSS-эталонами. Не подгонять «на глаз» без правки §s10 сначала.
 */

export const darkTokens: ThemeTokens = {
  scheme: "dark",
  // ink-1/2/3: §s10 строки 2804–2806.
  ink1: "#FFFFFF",
  ink2: "rgba(255,255,255,0.60)",
  ink3: "rgba(255,255,255,0.40)",
  // accent / accent-grad: §s10 строки 2807–2808.
  accent: "#8B5CF6",
  accentGrad: { angle: 135, colors: ["#8B5CF6", "#3B82F6"] },
  // bg-page: §s10 строка 2809 + CSS строка 24 (168° #161038→#241A5E→#13224E→#0A1130).
  bgPage: {
    angle: 168,
    colors: ["#161038", "#241A5E", "#13224E", "#0A1130"],
    locations: [0, 0.38, 0.68, 1],
  },
  // bg-blobs (§s10 строка 2810 + CSS строки 25–27): фиолет .5 / циан .3 /
  // роз .26 / — . Циан W42→W30 (CSS 25), роз W40→W26 (CSS 26), синий блоб
  // спрятан целиком (CSS 27 → background:none) — поэтому 4-го элемента нет.
  blobs: [
    { color: "rgba(124,92,255,0.5)", size: 380, top: -110, left: -80 },
    { color: "rgba(34,211,238,0.3)", size: 360, top: 250, right: -120 },
    { color: "rgba(244,114,182,0.26)", size: 380, top: 480, left: -110 },
  ],
  // glass-1: §s10 строка 2811 → 135° W13→W5, blur 24. CSS-оверрайды (строки
  // 28–36) переводят все светлые «160° W72→W46»-подобные в этот же стоп.
  glass1: { angle: 135, colors: ["rgba(255,255,255,0.13)", "rgba(255,255,255,0.05)"], blur: 24 },
  // glass-2 (тонкий): §s10 строка 2812 → 135° W11→W4, blur 20. CSS строка 37
  // подменяет светлое «160° W60→W40» на этот же стоп.
  glass2: { angle: 135, colors: ["rgba(255,255,255,0.11)", "rgba(255,255,255,0.04)"], blur: 20 },
  // glass-border: §s10 строка 2813 + CSS строки 54–61 (W78/W80/W85/W90 → W16/W18).
  // Общий бордер стекла — W16; для более крупных поверхностей (шторки/tab-bar)
  // локально повышается до W18 в собственном файле компонента (эталон CSS 60–61).
  glassBorder: "rgba(255,255,255,0.16)",
  // glass-inset: §s10 строка 2814 → inset 0 1 0 W22.
  glassInset: { y: 1, color: "rgba(255,255,255,0.22)" },
  // sh-card: §s10 строка 2815 → 0 12 32 rgba(4,6,20,.35).
  shCard: { x: 0, y: 12, blur: 32, color: "rgba(4,6,20,0.35)" },
  // sh-float: §s10 строка 2816 → 0 18 40 rgba(3,5,18,.50).
  shFloat: { x: 0, y: 18, blur: 40, color: "rgba(3,5,18,0.50)" },
  // sh-color: §s10 строка 2817 → glow 0 0 10 rgba(субъект,.50).
  shColor: (rgb) => ({ x: 0, y: 0, blur: 10, color: `rgba(${rgb},0.50)` }),
  // chip-bg / chip-bd: §s10 строка 2818 → rgba(цвет,.14) / rgba(цвет,.38).
