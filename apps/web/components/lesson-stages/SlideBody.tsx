...
function SlideContent({ slide, current, total, stageImageUrl }: { slide: LessonSlide; current: number; total: number; stageImageUrl?: string | null }) {
  ...
  /**
   * КАРТИНКА СЛАЙДА — ТОЛЬКО СВОЯ. 04.09.2026.
   *
   * Было: `slide.image_url ?? (current === 0 ? stageImageUrl : null)` — если у
   * слайда своей картинки нет, к ПЕРВОМУ слайду приклеивалась картинка
   * ЭТАПА. А она заказана под тему этапа целиком, и на вводном слайде
   * оказывалась не к месту: у 27 этапов из 39 с картинкой слайдов шесть-семь,
   * и все, кроме первого, шли пустыми, а первый показывал чужое.
   *
   * Теперь слайд показывает СВОЮ картинку или никакой. Своя у него есть, если
   * модель выбрала макет 'split' и написала image_prompt, — частоту этого мы
   * не меняем, её решает модель.
   *
   * Картинка этапа не пропала: она по-прежнему показывается там, где для неё
   * и место — на этапе без слайдов, блоком StageMedia.
   */
  const asideImage = slide.image_url ?? null;
  return (
