  ...
  return (
    <>
      ...
      {/* Карточка уровня — заглушка без БД. Промт 6.2.1: возвращена вниз
          сайдбара (order-реорганизация из 6.2 откачена).
          Промт 6.2.6 Часть 2: на обычном десктопе с показанным DemoBanner
          (жёлтая полоса сверху) космонавт уходил за нижний край — aside
          растягивается на h-screen через flex-stretch от родителя
          (AppShell), но DemoBanner добавляет свой 40px-спейсер ПЕРЕД
          AppShell как сосед, а не внутри него, так что h-screen-бокс
          AppShell фактически съезжает на 40px вниз и настолько же
          вылезает за низ вьюпорта — это первопричина живёт в
          app/(app)/layout.tsx, вне разрешённой для правки области
          (StudentSidebar.tsx). В рамках только этого файла: pb-8 → pb-16
          (32→64px, больше запаса в обычном случае без баннера) + sticky
          bottom-4 (тот самый "sticky bottom с safe отступом" из спеки) —
          блок остаётся видимым в нижних 16px вьюпорта даже если
          какая-то часть страницы всё же скроллится. */}
      <div
        className={cn(
          "sticky bottom-4 shrink-0 overflow-hidden rounded-2xl",
