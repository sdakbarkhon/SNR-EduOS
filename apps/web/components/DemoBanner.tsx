...
export function DemoBanner({ isDemo }: { isDemo: boolean }) {
  ...
  return (
    <>
      ...
      {/* Баннер рендерится ВНУТРИ ScaleWrapper (см. app/(app)/layout.tsx /
          app/teacher/layout.tsx), рядом с AppShell/TeacherShell как
          flex-сосед — ScaleWrapper's transform делает его containing block
          для потомков с position:fixed (CSS-спека), так что inset-x-0
          позиционируется относительно масштабированного 1920-холста, а не
          реального окна, и растягивается вместе со всем остальным без
          отдельного min-[1920px]-хака. */}
