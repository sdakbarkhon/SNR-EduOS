...
// Пачка A (хвост от 15774c8): на простых темах 3-А («Вывод текста на
// экран», «Первая программа на Python») Gemini регулярно давал код с < 3
// пропусков — естественно, «print("Привет")» в 8-12 строк не наберёт 4-6
// точек для пропуска без искусственного растягивания. С 3-й попытки промт
// становится настойчивее: явно требует несколько ОТДЕЛЬНЫХ print/переменных
// вместо одной короткой строки, чтобы физически было что пропускать.
function systemPromptFor(className, topic, spec, escalate = false) {
...
строка — сделай мини-сценарий (например, программа здоровается, представляет
предмет и учителя, считает что-то простое) так, чтобы гарантированно набралось
${spec.gaps} пропусков.`
    : "";
...
4 варианта (1 правильный + 3 правдоподобных ошибочных). ${spec.extra}${escalation}
...
async function main() {
  ...
  for (const group of groups) {
    ...
    // Догон партиями (Пачка A, хвост от 15774c8): homework не хранит topic
    // отдельной колонкой, поэтому при ЧАСТИЧНОМ покрытии (0 < existingCount <
    // topics.length) НЕЛЬЗЯ просто пройтись по всему topics — 3-5 уже
    // существующих строк сгенерировались бы ЗАНОВО поверх старых (дубли).
    // Единственный известный сейчас случай частичного покрытия — 3-А после
    // прошлой сессии (3 из 5): по содержанию существующих ДЗ («Простые
    // вычисления», «Переменные и типы данных», и «Фруктовый Счёт» — счёт
    // фруктов/стоимости/конфет, по теме это «Работа с числами») ни одна не
    // пересекается с «Вывод текста на экран» и «Первая программа на Python»
    // — они и остаются недостающими. Явный список безопаснее угадывания по
    // позиции в массиве (порядок успеха/неудачи в прошлом запуске зависел от
    // темы, а не от порядка). Для НЕИЗВЕСТНОГО случая частичного покрытия —
    // пропуск с предупреждением, а не риск дублей.
    let topicsToTry = topics;
    if ((existingCount ?? 0) > 0) {
      const KNOWN_MISSING = {
        "3-А класс": ["Вывод текста на экран", "Первая программа на Python"],
      };
      const missing = KNOWN_MISSING[group.name];
      if (!missing) {
        console.warn(`  ["${group.name}"] → частичное покрытие (${existingCount}/${topics.length}) без известного списка недостающих тем — ПРОПУСК во избежание дублей.`);
        homeworkSkipped += topics.length - (existingCount ?? 0);
        continue;
      }
      topicsToTry = missing;
      console.log(`  ["${group.name}"] → частичное покрытие (${existingCount}/${topics.length}), догоняю: ${missing.join(", ")}`);
    }
    ...
    for (const topic of topicsToTry) {
      ...
      for (let attempt = 0; attempt < 4 && !v; attempt++) {
        ...
        let response;
        try {
          response = await callGeminiWithRetry(systemPromptFor(group.name, topic, spec, attempt >= 2), `Класс: ${group.name}\nТема: ${topic}`);
        } catch (e) {
