...
// Пачка 4 — тот же increment_ai_usage() RPC (миграция 136), что
// apps/web/lib/ai/gemini-client.ts's bumpAiUsage() зовёт на каждый успешный
// вызов из веб-приложения. Таблица ai_usage_log — просто (day,
// requests_count, updated_at), БЕЗ user_id/role/endpoint — RPC ничего
// такого не принимает, обходить NOT NULL нечего. В отличие от клиента (там
// fire-and-forget — сбой счётчика не должен ронять чат) — здесь ждём и
// логируем ошибку явно: скрипт не UI, задержка в 1 RPC-вызов не критична,
// а тихий пропуск инкремента исказил бы видимый расход квоты сильнее, чем
// в интерактивном чате (тут одна сессия может съесть десятки вызовов).
