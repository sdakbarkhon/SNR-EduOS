...
export function HomeworkDetailView({ hw }: { hw: HomeworkWithSubmission }) {
  ...
  return (
    // Заход 2 сузил это до max-w-4xl (896px) ради читаемости длинных абзацев
    // (описание задания, текстовое поле сдачи) — но по факту оставлял
    // избыточные пустые поля по бокам (Большой фикс, доп. "Ширина в
    // проектах и заданиях"). max-w-6xl (1152px) — компромисс: заметно шире
    // прежнего узкого предела, но всё ещё уже полного каркаса (1600px),
    // строка текста не становится чрезмерно длинной.
