...
/** Частота прямого опроса статуса урока (страховка на случай, если
 *  Realtime-подписка промолчит, см. ниже). 13.09.2026, заход 0 — было 3000
 *  (3 с): на 200 учениках в 20 уроках это 2,6–27 ГБ/мес при лимите 5 ГБ,
 *  один этот опрос был способен съесть весь бесплатный трафик Supabase.
 *  Раз в 30 с — тот же запасной путь, трафик в десять раз меньше
 *  (resheniya_3.md, разведка 11.09.2026, часть Г). Подписка (см. ниже)
 *  не тронута — она и раньше была основным путём смены статуса. */
const LESSON_STATUS_POLL_MS = 30_000;

/** Сколько молчания сервера считаем потерей связи. Порог обязан быть
 *  заведомо выше LESSON_STATUS_POLL_MS — иначе при неудачном совпадении
 *  фаз опрос будет отвечать РЕЖЕ порога, и «выход с усилием» станет
 *  сниматься сам на каждом уроке. 13.09.2026 — было 15000 (5× от старых
 *  3000); при опросе раз в 30 с сохранён тот же пятикратный запас был бы
 *  150 с, это посчитали избыточным для аварийного снятия замка — взяли
 *  тройной запас: 90000 (3× от новых 30000). Замолчал дольше — узнать
 *  больше нечем, и «выход с усилием» у ученика снимается совсем. */
const LIVE_SYNC_STALE_MS = 90_000;
