...
export function TeacherGroupsView({ groups, homework, grades, attendance }: Props) {
          ...
          return (
            <Link key={group.id} href={`/teacher/groups/${group.id}`}
              ...
              <div className="mb-6 flex items-center gap-4">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[16px] text-xl font-bold text-white shadow-lg shadow-blue-500/20"
                  style={{ background: GRADIENTS[idx % GRADIENTS.length] }}>
                  {classBadge(group.name)}
                </div>
                <div className="min-w-0">
                  <h3 className="truncate text-lg font-bold leading-tight text-gray-900">{cfg.label}</h3>
                  <span className="mt-1 flex items-center text-xs font-semibold text-gray-500">
                    <Users className="mr-1 h-3.5 w-3.5" />
                    {studentCount} {d.teacher.groupStudents}
                  </span>
                </div>
              </div>

              <div className="mt-2 grid grid-cols-2 gap-4 border-t border-gray-100/50 pt-5">
                <div>
