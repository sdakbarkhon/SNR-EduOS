...
import { X, Plus, Trash2, Check, FileText, TestTube2, BookOpen, Code2, ChevronDown } from "lucide-react";
...
export function ClassworkModal({ open, onClose, lessonId, teacherId, groupId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: studentRows } = await db2
        ...
        .eq("group_id", groupId);
      const groupStudents = ((studentRows ?? []) as Array<{ student: { id: string; full_name: string; avatar_url: string | null } }>)
        .map((r) => r.student)
      ...
      if (cw) {
        ...
        setQuestions(cw.questions.map((q) => ({
  ...
  async function handleSave() {
    ...
    try {
      const cwId = await createClasswork(db as never, {
        lessonId, teacherId, title: title.trim(),
        description: description.trim() || undefined, workType,
      });
      if (workType === "test" && questions.length > 0) {
        await createClassworkQuestions(db as never, cwId,
          questions.map((q, i) => ({
  ...
  async function handleGrade(submissionId: string) {
    ...
    setGrades((prev) => ({ ...prev, [submissionId]: { grade: prev[submissionId]?.grade ?? "", comment: prev[submissionId]?.comment ?? "", saving: true } }));
    try {
      ...
      setGrades((prev) => ({ ...prev, [submissionId]: { grade: prev[submissionId]?.grade ?? "", comment: prev[submissionId]?.comment ?? "", saving: false } }));
    }
  }
        ...
        <div className="flex border-b border-[var(--border)] px-6">
          {(["task", "submissions"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`py-3 pr-4 text-sm font-medium border-b-2 transition-colors ${tab === t
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="space-y-5">
                    ...
                    <div key={qi} className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-4 space-y-3">
                      ...
                      {q.options.map((opt, oi) => (
                        <div key={oi} className="flex items-center gap-2 pl-7">
                          <button onClick={() => updateQuestion(qi, { correct_index: oi })}
              ...
              {/* Existing classwork questions preview */}
                    ...
                    <div key={s.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
                      ...
                      {hasSubmission && s.test_score != null && (
                        <p className="text-xs text-[var(--text-3)] mb-2">
                          {d.teacher.classworkTestScore.replace("{score}", String(s.test_score)).replace("{max}", String(s.test_max ?? "?"))}
                        </p>
                      )}

                      {hasSubmission && s.text_answer && (
                        <p className="text-xs text-[var(--text-2)] bg-[var(--surface-1)] rounded-lg p-2 mb-2 line-clamp-2">{s.text_answer}</p>
                      )}

                      {hasSubmission && (
                        <details className="group">
                          <summary className="flex items-center gap-1 text-xs text-[var(--text-3)] cursor-pointer select-none list-none">
                            <ChevronDown className="w-3.5 h-3.5 transition-transform group-open:rotate-180" />
                            {d.teacher.classworkGradeLabel}
                          </summary>
                          <div className="mt-2 space-y-2">
                            <div className="flex gap-2">
                              <input type="number" min={1} max={5}
                                value={g?.grade ?? ""}
                                onChange={(e) => setGrades((prev) => ({ ...prev, [s.id]: { grade: e.target.value, comment: prev[s.id]?.comment ?? "", saving: false } }))}
                              ...
                              <input type="text"
                                value={g?.comment ?? ""}
                                onChange={(e) => setGrades((prev) => ({ ...prev, [s.id]: { grade: prev[s.id]?.grade ?? "", comment: e.target.value, saving: false } }))}
                                placeholder={d.teacher.classworkCommentLabel}
