...
// 18.08.2026 — было force-dynamic, и каждый заход на экран входа стоил
// полного круга: запрос к базе плюс подпись ссылок на логотипы. Замер в
// браузере — 3510 мс на холодном первом запросе. Список школ меняется раз в
// месяц, а ссылка на логотип подписана на час, поэтому пять минут кэша
// безопасны: подпись не успеет протухнуть, а новая школа появится через пять
// минут после создания.
