 ...
 * ПРИЧИНОЙ словами, а файла не остаётся: он пишется последним и только после
 * того, как темы уже есть. Полуготового файла не бывает — либо он записан
 * целиком и путь проставлен, либо его нет вовсе.
 *
 * ═══ ИДЕМПОТЕНТНОСТЬ ══════════════════════════════════════════════════════
 *
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // ── Файл ───────────────────────────────────────────────────────────────
    // Пишется ПОСЛЕДНИМ и один раз: до этого места мусора не остаётся, а
    // после — путь проставляется в той же записи, что и «готово».
    await шаг("file", 90);
