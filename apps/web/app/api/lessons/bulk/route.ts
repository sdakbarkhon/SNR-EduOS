...
type Body = {
  ...
  /** Выходные и праздники, «YYYY-MM-DD». Шлют оба вызывающих: учебный план и
   *  массовое создание на экране уроков (04.09.2026 — там тоже темы плана и
   *  период по их числу, значит праздник не съедает урок, а сдвигает его).
   *  Причина праздника сюда не едет: она для человека, серверу нужны даты. */
  holidays?: string[];
  from?: string;
  to?: string;
  useTopics?: boolean;
  ...
  for (const l of toCreate) {
    try {
      const lesson = await createLesson(db, {
        ...
        room,
        // ОПИСАНИЕ ТЕМЫ ПЛАНА СЮДА БОЛЬШЕ НЕ ЕДЕТ. 04.09.2026.
        //
        // Копировалось `description: <описание темы>`, и ученик читал его на
        // экране урока: «Изучение основных понятий программирования, его
        // полезности для детей и понимание принципов работы с данной книгой».
        // Это методическая заметка ИЗ ПЛАНА, написанная для учителя, — и
        // адресована она не ученику.
        //
        // Поле урока «Описание / цель» остаётся, и оно как раз ученическое:
        // подсказка под ним прямо говорит «Что ученики должны узнать». Пусть
        // там будет то, что учитель написал детям, а не то, что машина
        // переписала из плана. Само описание темы никуда не делось — оно
        // по-прежнему в плане, где его и читает учитель.
        title: l.topicTitle,
        description: null,
        subjectId,
