...
/**
 * Комната поддержки родителя — или null.
 *
 * ОТКАТА НА КУРАТОРА ЗДЕСЬ БОЛЬШЕ НЕТ (29.08.2026). Раньше при отсутствии
 * поддержки функция отдавала direct-тред классного руководителя, и пункт
 * «Помощь и поддержка» открывал переписку С УЧИТЕЛЕМ, подписанную как
 * поддержка. Пока вида «support» не существовало, это был осознанный выбор
 * — лучше живой куратор, чем ничего. Теперь это подмена собеседника:
 * человек думает, что пишет школе, а пишет учителю, и тот получает вопрос
 * про справки и оплаты, который к нему не относится.
 *
 * Нет комнаты — возвращаем null, и экран предлагает её ЗАВЕСТИ. Подсовывать
 * другую переписку вместо отсутствующей нельзя.
 *
 * ИСТОРИЯ ОТКАТОВ, чтобы её не завели заново. Первым здесь стоял
 * `threads.find(t => t.kind === "group")` — ЛЮБОЙ групповой чат класса; он
 * срабатывал всегда и открывал именно то, что было прямо запрещено. Его
 * заменили на куратора, теперь убран и куратор. Никакого «ближайшего
 * подходящего» треда у поддержки нет: у неё есть свой вид чата.
 */
export function pickSupportThread(threads: ParentThreadVM[]): ParentThreadVM | null {
  return threads.find((t) => t.isSupport) ?? null;
}
