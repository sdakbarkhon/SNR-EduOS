/**
 * ВЕБ-КОПИЯ (Блок 7.1). Файл скопирован ДОСЛОВНО из
 * apps/mobile-parent/src/data/index.ts (ветка feat/mobile-parent-redesign),
 * чтобы веб-родитель показывал ровно те же числа и тексты, что Expo Go.
 * Правки при копировании — ТОЛЬКО постфиксы `!` на чтениях по индексу:
 * в apps/web включён `noUncheckedIndexedAccess`, в apps/mobile-parent — нет.
 * Все такие чтения безопасны по построению (массивы-литералы непустые, а
 * инварианты вида CHILDREN.length === WALLETS.length описаны в фикстурах).
 * Логика, сигнатуры и данные не менялись — файл должен оставаться
 * построчно сравнимым с мобильным оригиналом.
 *
