...
/**
 * Столько работ ученик держит одновременно.
 *
 * Живёт ЗДЕСЬ, а не в actions.ts, потому что число нужно обеим сторонам:
 * server action им отказывает, а экран «Мои работы» показывает «N из 20».
 * Экспортировать его из actions.ts нельзя технически — файл помечен
 * "use server", а такие файлы могут экспортировать только асинхронные
 * функции. Вторую копию числа заводить тем более нельзя: в этом проекте
 * копии расходились семь раз.
 *
 * Своё число, не общий SANDBOX_PROJECT_LIMIT из packages/core: там текст
 * кода, здесь мегабайтные файлы.
 */
export const SCRATCH_PROJECT_LIMIT = 20;
