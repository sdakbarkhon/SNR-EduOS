...
// Live-tested (см. resheniya_2.md, ЧАСТЬ 8): на текущем GEMINI_API_KEY (free
// tier) gemini-2.5-pro отвечает 429 с "limit: 0" — это НЕ временное
// исчерпание квоты (там был бы ненулевой limit), а структурное отсутствие
// Pro-доступа на бесплатном тарифе. Обычный backoff-retry здесь бесполезен
// (три попытки ничего не изменят), поэтому такие ответы триггерят
// немедленный, разовый downgrade на Flash внутри той же попытки, вместо
// того чтобы просто провалить запрос — фича продолжает работать (чуть хуже
// качеством) вместо жёсткого отказа. При апгрейде GEMINI_API_KEY на платный
// тариф это условие просто перестанет срабатывать без правок кода.
