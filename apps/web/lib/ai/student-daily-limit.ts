...
// часа, и окно считается ОТ МОМЕНТА ИСЧЕРПАНИЯ — потратил двадцатый, ждёшь
// ровно два часа с этого мгновения, потом снова полные двадцать.
//
// Считает по-прежнему база — теперь fn_ai_window_state (миграция 253). Разбор,
// почему окно выводится из самих сообщений и новой таблицы не нужно, — в шапке
// той миграции. Заморозка времени школы на счёт не влияет: сюда её не
// подмешиваем.
//
...
export type StudentAiUsage = {
  ...
  remaining: number;
  limit: number;
  /** Запас исчерпан до этого момента (ISO). null — запас есть. */
