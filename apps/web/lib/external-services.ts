...
// 06.09.2026 — КАРТЫ «ПРЕДМЕТ → СЕРВИСЫ» ЗДЕСЬ БОЛЬШЕ НЕТ.
//
// Она ключевалась русским названием и знала пять имён; предмет, которого в
// ней нет, получал четыре сервиса из четырнадцати. Теперь набор лежит у
// предмета справочника (`school_subjects.services`, миграция 258) и задаётся
// галочками — читает его lib/subject-services.ts.
//
// Прежняя запись (Этап 5.4) — filters the 12-service picker by subject
