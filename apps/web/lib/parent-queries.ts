/**
 * Запросы родительских экранов к настоящей базе.
 *
 * ВНИМАНИЕ, ЭТО НЕ МУСОР. Денежная часть этого файла — childBalance,
 * childInvoices, childBalanceEntries, childPaymentsSummary — СЕГОДНЯ НИКЕМ
 * НЕ ВЫЗЫВАЕТСЯ и ни на один экран не попадает. Так и задумано.
 *
 * 27.08.2026 экраны оплат родителя перевели с витрины на настоящие счета и
 * баланс (миграции 227/229/230). 28.08.2026 заказчик это отменил: родительская
 * часть — витрина будущего продукта, её показывают людям именно такой, и
 * настоящих данных там быть не должно, пока не подключена касса. Экраны
 * вернулись на заготовки, а этот слой оставлен лежать целиком: работа
 * сделана и проверена, при подключении кассы он понадобится как есть.
 *
 * Правило про витрину — раздел 0 в CLAUDE_CHAT_HANDOFF.md. Список экранов,
 * которые придётся переключить обратно, — в resheniya_3.md, раздел
 * «Отмена: витрина родителя возвращена».
 *
 * Остальные функции файла (уроки, оценки, посещаемость, материалы) живые и
 * работают — отключены только денежные.
 */
...
/**
 * ДЕМО ЛИ СМОТРЯЩИЙ. Заход 1 по оплатам, 30.08.2026.
 *
 * Один булев из `schools.is_demo` — см. подробное обоснование у поля `isDemo`
 * в lib/parent-context.ts. Отдельная функция по той же причине, что и
 * getSelectedChildId выше: вызывающему не нужен весь контекст ради одного
 * поля.
 *
 * СЕТИ НЕ ДОБАВЛЯЕТ. getParentContext обёрнут в cache() и уже вызван в
 * layout.tsx этого же запроса — здесь это попадание в кеш, а не round-trip.
 *
 * ВЕТВЛЕНИЙ ПО НЕМУ ПОКА НЕТ НИ ОДНОГО, и это осознанно: заход 1 только
 * доводит признак до экранов оплат, заготовка остаётся видна обоим. Ветвиться
 * будут заходы 2 и дальше — там, где деньги переезжают на настоящие счета.
 */
export const parentIsDemo = cache(async (): Promise<boolean> => {
  const ctx = await getParentContext();
