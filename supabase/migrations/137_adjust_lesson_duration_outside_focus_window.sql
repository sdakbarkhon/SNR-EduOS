...
--
-- Тот же SQL, что был выпущен раньше как apps/web/scripts/
-- shrink-lessons-out-of-window.sql (Пачка 4, Задача A) — здесь та же
-- логика оформлена как пронумерованная миграция по просьбе заказчика.
-- Примечание: `updated_at = now()` из исходной формулировки убран — у
-- lessons такой колонки нет (подтверждено live-запросом к hosted БД,
-- PostgREST: "column lessons.updated_at does not exist"), с ней UPDATE
-- упал бы в Dashboard SQL Editor.
